(function () {
  ...
  const TZ_MS = 7 * 3600 * 1000; // Asia/Ho_Chi_Minh, no DST
  ...
  function toEpochVN(y, m, d, h, min) {
    return Date.UTC(y, m - 1, d, h || 0, min || 0, 0) - TZ_MS;
  }
  ...
  function render() {
    const now = nowVN();
    document.getElementById("clock").textContent = `Hôm nay: ${WEEKDAY_LABELS[now.dow === 0 ? 6 : now.dow - 1]} · ${pad(now.h)}:${pad(now.min)}:${pad(now.s)}`;
    document.getElementById("updatedAt").textContent = window.SCHEDULE && window.SCHEDULE.updated_at ? `Cập nhật: ${window.SCHEDULE.updated_at}` : "";
    ...
    if (next) {
      ...
      if (st.key === "ongoing") {
        ...
        banner.innerHTML = `
          <div class="nb-label">Đang học ngay bây giờ</div>
          <div class="nb-title">${next.ev.ten_mon}</div>
          <div class="nb-sub">${next.ev.phong} · ${next.ev.gio_bat_dau} – ${next.ev.gio_ket_thuc}</div>
          <div class="nb-count">Còn lại: ${fmtDuration(st.ms)}</div>`;
      } else {
        banner.className = "next-banner upcoming";
        const dt = parseISO(next.occ.dateISO);
        banner.innerHTML = `
          <div class="nb-label">Tiết tiếp theo</div>
          <div class="nb-title">${next.ev.ten_mon}</div>
          <div class="nb-sub">${WEEKDAY_LABELS[next.ev.thu_idx]} ${pad(dt.d)}/${pad(dt.m)} · ${next.ev.gio_bat_dau} – ${next.ev.gio_ket_thuc} · ${next.ev.phong}</div>
          <div class="nb-count">Còn: ${fmtDuration(st.ms)}</div>`;
      }
    } else {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      dayEvents.forEach(ev => {
        ...
        card.innerHTML = `
          <div class="class-title">${ev.ten_mon}</div>
          <div class="class-time">Tiết ${ev.tiet_start}-${ev.tiet_end} · ${ev.gio_bat_dau}–${ev.gio_ket_thuc}</div>
          <div class="class-loc">${ev.phong}${ev.cach_tuan ? " · Cách tuần" : ""}</div>
          <span class="class-status ${st.key}">${statusLabel}</span>`;
